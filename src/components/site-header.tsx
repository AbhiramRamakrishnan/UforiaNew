...
export function SiteHeader() {
  ...
  return (
    ...
    >
      <nav className={`flex w-full max-w-6xl items-center justify-between gap-6 rounded-full px-5 py-2.5 transition-all duration-500 ${scrolled ? "glass shadow-luxe" : "bg-transparent"}`}>
        <Link to="/" className="inline-flex items-center rounded-full border border-white/80 bg-white px-3.5 py-1.75 shadow-md transition-colors hover:bg-white">
          <img src="/full_logo_bgremoved.png" alt="Fynora" className="h-6 w-auto md:h-7" />
          ...
          {links.map((l) => {
            ...
            return (
              <li key={l.to}>
                <Link
                  to={l.to}
                  className={`relative rounded-full px-3.5 py-1.5 text-sm transition-colors ${active ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {active && (
                    <motion.span
                      layoutId="nav-pill"
